...
impl OriginalBotKind {
    fn convert_name(&self, name: &str) -> String {
        match self {
            OriginalBotKind::Pipisa => name.chars().take(13).collect(),
...
pub async fn import_cmd_handler(
    ...
    let answer = match check_and_parse_message(&bot, &msg).await {
        Ok(parsed) => match import_impl(&repos, msg.chat.id, parsed).await {
            Ok(r) => {
                ...
                let not_found = r
                    ...
                    .map(|name| {
                        t!(
                            "commands.import.result.line.not_found",
                            locale = &lang_code,
                            name = name.escaped()
                        )
                    })
            ...
            Err(e) => {
                if let Some(InvalidLines(lines)) = e.downcast_ref() {
                    log::error!("Invalid lines: {lines:?}");
                    let invalid_lines = lines
                        .iter()
                        .map(|line| {
                            t!(
                                "commands.import.errors.invalid_lines.line",
                                locale = &lang_code,
                                line = line
                            )
                        })
                        ...
                        .join("\n");
                    t!(
                        "commands.import.errors.invalid_lines.template",
                        locale = &lang_code,
        ...
        Err(BeforeImportCheckErrors::NotReply) => {
            let origin_bots = ORIGINAL_BOT_USERNAMES
                .map(|name| format!("@{name}"))
                .join(", ");
            t!(
                "commands.import.errors.not_reply",
                locale = &lang_code,
                origin_bots = origin_bots
            )
            .to_string()
        }
        Err(e) => {
...
async fn import_impl(
    ...
    let members: HashMap<String, ChatMember> = repos
        .users
        .get_chat_members(&chat_id_kind)
        .await?
        .into_iter()
        .map(|m| {
    ...
    let top: Vec<OriginalUser> = top.into_iter().flatten().filter_map(map_user).collect();
    let (existing, not_existing): (Vec<OriginalUser>, Vec<OriginalUser>) = top
        .into_iter()
        .partition(|u| member_names.contains(u.name.as_ref()));
    let existing: Vec<UserInfo> = existing
        .into_iter()
        .map(|u| {
        ...
        .collect();
    let not_found = not_existing.into_iter().map(|u| u.name).collect();

    let imported_uids: HashSet<UserId> = repos
        .import
    ...
    let (already_present, to_import): (Vec<UserInfo>, Vec<UserInfo>) = existing
        .into_iter()
        .partition(|u| imported_uids.contains(&u.uid));

    let users: Vec<repo::ExternalUser> = to_import
        .iter()
...
mod tests {
    ...
    fn original_bot_kind_try_from() {
        let check = |variant: &str, kind| {
            ...
            let invalid_variants = valid_variants.into_iter().map(|v| {
                v.strip_suffix("_bot")
                    .or_else(|| v.strip_suffix("bot"))
                    .expect("no 'bot' suffix")
            });
            assert!(invalid_variants
                .into_iter()
                .all(|v| OriginalBotKind::try_from(v).is_err()));
        };
