...
use crate::handlers::{reply_html, FromRefs, HandlerResult};
...
async fn personal_stats_impl(
    ...
    repos.personal_stats.get(from_refs.0.id).await.map(|stats| {
        t!(
...
pub(crate) async fn chat_stats_impl(
    ...
    let lang_code = LanguageCode::from_user(from_refs.0);
    let (length, position) = repos
        .dicks
        .fetch_dick(from_refs.0.id, &from_refs.1.kind())
        .await?
