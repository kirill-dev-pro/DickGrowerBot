...
pub trait CallbackDataWithPrefix<E = InvalidCallbackData>:
    TryFrom<String, Error = E> + std::fmt::Display
where
...
pub fn get_params_for_message_edit(
    q: &CallbackQuery,
) -> Result<EditMessageReqParamsKind, &'static str> {
    q.message
        .as_ref()
        .map(|m| EditMessageReqParamsKind::Chat(m.chat().id, m.id()))
        .or(q.inline_message_id.as_ref().map(|inline_message_id| {
...
pub async fn prepare_callback_answer_params(
    ...
    let res = if query.from.id != user_id {
        ...
        answer
            .text
            .replace(t!("inline.callback.errors.another_user", locale = &lang_code).to_string());
        answer.await?;
...
macro_rules! check_invoked_by_owner_and_get_answer_params {
    ($bot:ident, $query:ident, $user_id:expr) => {
        match $crate::handlers::utils::callbacks::prepare_callback_answer_params(
            &$bot, &$query, $user_id,
        )
        .await?
        {
            $crate::handlers::utils::callbacks::CallbackAnswerParams::Answer {
                answer,
                lang_code,
            } => (answer, lang_code),
