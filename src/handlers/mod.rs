...
use teloxide::types::{
    CallbackQuery, InlineKeyboardButton, InlineKeyboardMarkup, Message, ReplyParameters,
};
use teloxide::Bot;
    ...
    pub async fn apply(self, bot: Bot, callback_query: CallbackQuery) -> anyhow::Result<()> {
        ...
        match self {
            ...
            CallbackResult::ShowError(err) => {
                answer_req.text(err).show_alert(true).await?;
            }
        };
        Ok(())
    }
        ...
        match self {
            HandlerImplResult::WithKeyboard { buttons, .. } => {
                let buttons = buttons.iter().map(|btn| {
                    InlineKeyboardButton::callback(btn.title.clone(), btn.data.to_data_string())
                });
...
pub fn reply_html<T: Into<String>>(bot: Bot, msg: &Message, answer: T) -> JsonRequest<SendMessage> {
    ...
    if msg.chat.is_group() || msg.chat.is_supergroup() {
        answer
            .reply_parameters
            .replace(ReplyParameters::new(msg.id));
    }
    answer
}
...
pub async fn send_error_callback_answer(
    bot: Bot,
    query: CallbackQuery,
    tr_key: &str,
) -> HandlerResult {
...
pub mod checks {
    use super::{reply_html, HandlerResult};
    use crate::domain::LanguageCode;
    use rust_i18n::t;
