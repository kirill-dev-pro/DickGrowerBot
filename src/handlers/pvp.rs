...
use crate::handlers::utils::callbacks::{
    CallbackDataWithPrefix, InvalidCallbackDataBuilder, NewLayoutValue,
};
use crate::handlers::utils::locks::LockCallbackServiceFacade;
use crate::handlers::{
    reply_html, send_error_callback_answer, utils, CallbackResult, HandlerResult,
};
...
use teloxide::types::{
    CallbackQuery, ChatId, ChosenInlineResult, InlineKeyboardButton, InlineKeyboardMarkup,
    InlineQuery, InlineQueryResult, InlineQueryResultArticle, InputMessageContent,
    InputMessageContentText, Message, ParseMode, ReplyMarkup, User, UserId,
};
use teloxide::Bot;
...
pub async fn cmd_handler(
    ...
    let user = msg
        .from
        .as_ref()
        .ok_or(anyhow!("no FROM field in the PVP command handler"))?
        .into();
...
pub async fn cmd_handler_no_args(bot: Bot, msg: Message) -> HandlerResult {
    ...
    reply_html!(
        bot,
        msg,
        t!("commands.pvp.errors.no_args", locale = &lang_code)
    );
    Ok(())
}
...
pub async fn inline_handler(bot: Bot, query: InlineQuery) -> HandlerResult {
    ...
    let mut answer = bot
        .answer_inline_query(&query.id, vec![res.clone()])
        .is_personal(true);
    if cfg!(debug_assertions) {
        answer.cache_time.replace(1);
    }
    answer.await.context(format!(
        "couldn't answer a callback query {query:?} with {res:?}"
    ))?;
    Ok(())
}
...
pub async fn callback_handler(
    ...
    let chat_id: ChatIdPartiality = query
        .message
        .as_ref()
        .map(|msg| msg.chat().id)
        .or_else(|| {
    ...
    let _battle_guard = match battle_locker.try_lock(&callback_data) {
        Some(lock) => lock,
        None => {
            return send_error_callback_answer(
                bot,
                query,
                "commands.pvp.errors.battle_already_in_progress",
            )
            .await
        }
    };
    ...
    let attack_result = pvp_impl_attack(
        params,
        callback_data.initiator,
        query.from.clone().into(),
        callback_data.bet,
    )
    .await?;
...
pub(crate) async fn pvp_impl_start(
    ...
    let enough = p
        .repos
        .dicks
        .check_dick(&p.chat_id.kind(), initiator.uid, bet)
        .await?;
    log::debug!(
        "Starting a PvP for {} in the chat with id = {} (bet = {bet}, enough = {enough})...",
        initiator.uid,
        p.chat_id
    );
    ...
    let data = if enough {
        ...
        (
            t!(
                "commands.pvp.errors.not_enough.initiator",
                locale = &p.lang_code
            )
            .to_string(),
            None,
...
async fn pvp_impl_attack(
    ...
    let (enough_initiator, enough_acceptor) = join!(
        ...
        p.repos.dicks.check_dick(
            &chat_id_kind,
            acceptor.uid,
            if p.features.check_acceptor_length {
                bet
            } else {
                0
            }
    ...
    let result = if enough_initiator && enough_acceptor {
        ...
        let (loser_res, winner_res) = p
            .repos
            .dicks
            .move_length(&p.chat_id, loser, winner, bet)
            .await?;

        let battle_stats = p
            .repos
            .pvp_stats
            .send_battle_result(&p.chat_id.kind(), winner, loser, bet)
            .await
            .inspect_err(|e| {
                log::error!(
                    "couldn't send users' battle statistics for winner ({}) and loser ({}): {}",
                    winner,
                    loser,
                    e
                )
            ...
            .map(
                |BattleStats {
                    ...
                    let mut stats_str = t!(
                        ...
                        loser_win_rate = loser_stats.win_rate_formatted(),
        ...
        let (winner_res, withheld_part) = pay_for_loan_if_needed(&p, winner, bet)
            .await
        ...
        let main_part = t!(
            ...
            locale = &p.lang_code,
            winner_name = winner_info.name.escaped(),
            winner_length = winner_res.new_length,
            loser_length = loser_res.new_length,
            bet = bet
        );
        let text = if let (Some(winner_pos), Some(loser_pos)) =
            (winner_res.pos_in_top, loser_res.pos_in_top)
        {
            let winner_pos = t!(
                "commands.pvp.results.position.winner",
                locale = &p.lang_code,
                name = winner_info.name.escaped(),
                pos = winner_pos
            );
            let loser_pos = t!(
                "commands.pvp.results.position.loser",
                locale = &p.lang_code,
                name = loser_info.name.escaped(),
                pos = loser_pos
            );
            format!("{main_part}\n\n{winner_pos}\n{loser_pos}")
        } else {
        ...
        let text = t!(
            "commands.pvp.errors.not_enough.acceptor",
            locale = &p.lang_code
        )
        .to_string();
...
async fn pay_for_loan_if_needed(
    ...
    winner_id: UserId,
    award: u16,
) -> anyhow::Result<Option<(GrowthResult, u16)>> {
    let chat_id_kind = p.chat_id.kind();
    let loan = match p
        .repos
        .loans
        .get_active_loan(winner_id, &chat_id_kind)
        .await?
    {
        Some(loan) => loan,
        None => return Ok(None),
    ...
    let growth_res = p
        .repos
        .dicks
        .grow_no_attempts_check(&chat_id_kind, winner_id, withheld)
        .await?;
