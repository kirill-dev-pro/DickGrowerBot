...
use crate::handlers::{
    reply_html, try_resolve_chat_id, CallbackButton, FromRefs, HandlerImplResult, HandlerResult,
};
...
pub(crate) async fn loan_impl(
    ...
    if let Some(Loan { debt, .. }) = maybe_loan {
        if !config.features.multiple_loans {
            let left_to_pay =
                t!("commands.loan.debt", locale = &lang_code, debt = debt).to_string();
    ...
    let btn_agree = CallbackButton::new(
        t!(
            "commands.loan.confirmation.buttons.agree",
            locale = &lang_code
        )
        .to_string(),
        LoanCallbackData {
    ...
    let btn_disagree = CallbackButton::new(
        t!(
            "commands.loan.confirmation.buttons.disagree",
            locale = &lang_code
        )
        .to_string(),
        LoanCallbackData {
...
pub async fn callback_handler(
    ...
    let (mut answer, lang_code) =
        check_invoked_by_owner_and_get_answer_params!(bot, query, data.uid);
    ...
    match data.action {
        ...
        LoanCallbackAction::Confirmed {
            value,
            payout_ratio,
        } if payout_ratio == config.loan_payout_ratio => {
            ...
            match edit_msg_params {
                EditMessageReqParamsKind::Chat(chat_id, message_id) => {
                    ...
                    bot.edit_message_text(chat_id, message_id, updated_text)
                        .await?;
                }
                EditMessageReqParamsKind::Inline {
                    chat_instance,
                    inline_message_id,
                } => {
                    ...
                    bot.edit_message_text_inline(inline_message_id, updated_text)
                        .await?;
                }
            }
        }
        LoanCallbackAction::Confirmed { .. } => {
            let updated_text = t!(
                "commands.loan.callback.payout_ratio_changed",
                locale = &lang_code
            );
            match edit_msg_params {
                EditMessageReqParamsKind::Chat(chat_id, message_id) => {
        ...
        LoanCallbackAction::Refused => {
            ...
            match edit_msg_params {
                EditMessageReqParamsKind::Chat(chat_id, message_id) => {
                    let unable_to_delete_message = bot
                        .delete_message(chat_id, message_id)
                        .await
                        .inspect_err(|e| {
                            log::error!("Unable to delete a loan request message: {e}")
                        })
                        .is_err();
...
impl TryFrom<String> for LoanCallbackData {
    ...
    fn try_from(data: String) -> Result<Self, Self::Error> {
        ...
        let action = parts.next().ok_or_else(|| err.missing_part("action"))?;
...
mod test {
    ...
    fn test_parse() {
        ...
        let [cd_confirmed, cd_refused] =
            get_strings(uid, value, payout_ratio).map(build_callback_query);
        {
