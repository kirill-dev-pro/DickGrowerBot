...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let handler = dptree::entry()
        .branch(
            Update::filter_message()
                ...
                .endpoint(handlers::start_cmd_handler),
        )
        .branch(
            Update::filter_message()
                .filter_command::<HelpCommands>()
                .endpoint(handlers::help_cmd_handler),
        )
        .branch(
            Update::filter_message()
                .filter_command::<PrivacyCommands>()
                .endpoint(handlers::privacy_cmd_handler),
        )
        .branch(
            Update::filter_message()
                .filter_command::<DickCommands>()
                .filter(checks::is_group_chat)
                .endpoint(handlers::dick_cmd_handler),
        )
        .branch(
            Update::filter_message()
                .filter_command::<DickOfDayCommands>()
                .filter(checks::is_group_chat)
                .endpoint(handlers::dod_cmd_handler),
        )
        .branch(
            Update::filter_message()
                .filter_command::<BattleCommands>()
                .filter(checks::is_group_chat)
                .endpoint(handlers::pvp::cmd_handler),
        )
        .branch(
            Update::filter_message()
                .filter_command::<BattleCommandsNoArgs>()
                .filter(checks::is_group_chat)
                .endpoint(handlers::pvp::cmd_handler_no_args),
        )
        .branch(
            Update::filter_message()
                .filter_command::<StatsCommands>()
                .endpoint(handlers::stats::cmd_handler),
        )
        .branch(
            Update::filter_message()
                .filter_command::<LoanCommands>()
                .filter(checks::is_group_chat)
                .endpoint(handlers::loan::cmd_handler),
        )
        .branch(
            Update::filter_message()
        ...
        .branch(
            Update::filter_message()
                .filter_command::<PromoCommands>()
                .filter(checks::is_not_group_chat)
                .enter_dialogue::<Message, InMemStorage<PromoCommandState>, PromoCommandState>()
                .branch(
                    dptree::case![PromoCommandState::Start].endpoint(handlers::promo_cmd_handler),
                ),
        )
        .branch(
            Update::filter_message()
                .enter_dialogue::<Message, InMemStorage<PromoCommandState>, PromoCommandState>()
                .branch(
                    dptree::case![PromoCommandState::Requested]
                        .endpoint(handlers::promo_requested_handler),
                ),
        )
        .branch(
            Update::filter_message()
                .filter(checks::is_not_group_chat)
                .endpoint(checks::handle_not_group_chat),
        )
        .branch(
            Update::filter_inline_query()
                .filter(checks::inline::is_group_chat)
                .filter(handlers::pvp::inline_filter)
                .endpoint(handlers::pvp::inline_handler),
        )
        .branch(
            Update::filter_inline_query()
                .filter(handlers::promo_inline_filter)
                .endpoint(handlers::promo_inline_handler),
        )
        .branch(
            Update::filter_inline_query()
                .filter(checks::inline::is_group_chat)
                .endpoint(handlers::inline_handler),
        )
        .branch(
            Update::filter_inline_query()
                .filter(checks::inline::is_not_group_chat)
                .endpoint(checks::inline::handle_not_group_chat),
        )
        .branch(
            Update::filter_chosen_inline_result()
                .filter(handlers::pvp::chosen_inline_result_filter)
                .endpoint(handlers::pvp::inline_chosen_handler),
        )
        .branch(Update::filter_chosen_inline_result().endpoint(handlers::inline_chosen_handler))
        .branch(
            Update::filter_callback_query()
                .filter(handlers::page_callback_filter)
                .endpoint(handlers::page_callback_handler),
        )
        .branch(
            Update::filter_callback_query()
                .filter(handlers::pvp::callback_filter)
                .endpoint(handlers::pvp::callback_handler),
        )
        .branch(
            Update::filter_callback_query()
                .filter(handlers::loan::callback_filter)
                .endpoint(handlers::loan::callback_handler),
        )
    ...
    let help_context = config::build_context_for_help_messages(
        me,
        &incrementor,
        &handlers::ORIGINAL_BOT_USERNAMES,
    )?;
    ...
    match webhook_url {
        Some(url) => {
            ...
            let (mut listener, stop_flag, bot_router) =
                axum_to_router(bot.clone(), Options::new(addr, url)).await?;
