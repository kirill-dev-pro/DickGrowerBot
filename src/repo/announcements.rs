...
impl Announcements {
    pub async fn get_new(
        ...
        let maybe_announcement = match self.announcements.get(lang_code) {
            Some(announcement)
                if self
                    .check_conditions(chat_id, announcement, lang_code)
                    .await? =>
            {
    ...
    async fn check_conditions(
        ...
        let res = match self.get(chat_id_kind, lang_code).await? {
            ...
            Some(entity) if entity.hash[..] != announcement.hash[..] => {
                self.update(entity.chat_id, lang_code, &announcement.hash)
                    .await?;
                true
            }
            Some(entity) if entity.times_shown >= self.announcements.max_shows => false,
            Some(entity) => {
                self.increment_times_shown(entity.chat_id, lang_code)
                    .await?;
                true
            }
            None => {
                self.create(chat_id_kind, lang_code, &announcement.hash)
                    .await?;
                true
            }
        };
    ...
    async fn create(
        ...
        sqlx::query!(
            "INSERT INTO Announcements (chat_id, language, hash, times_shown) VALUES (
                ...
                $2, $3, 1)",
            chat_id_kind.value() as String,
            lang_code.to_supported_language() as SupportedLanguage,
            hash
        )
        .execute(&self.pool)
        .await
        ...
        .context(format!(
            "couldn't create the announcement for {chat_id_kind}, {lang_code:?}, {hash:?}"
        ))
    }

    async fn increment_times_shown(
        &self,
        chat_id: ChatIdInternal,
        lang_code: &LanguageCode,
    ) -> anyhow::Result<()> {
