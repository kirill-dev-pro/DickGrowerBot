...
impl Dicks {
    ...
    pub async fn get_top(
        ...
        .await
        .context(format!(
            "couldn't get the top of {chat_id} with offset = {offset} and limit = {limit}"
        ))
    }
    ...
    pub async fn set_dod_winner(
        ...
        let new_length = match Self::grow_no_attempts_check_internal(
            &mut *tx,
            internal_chat_id,
            uid,
            bonus as i32,
        ...
        let pos_in_top = self.get_position_in_top(internal_chat_id, uid).await?;
        Ok(Some(GrowthResult {
            new_length,
            pos_in_top,
        }))
    }

    pub async fn check_dick(
        &self,
        chat_id: &ChatIdKind,
        user_id: UserId,
        length: u16,
        ...
        sqlx::query_scalar!(
            r#"SELECT length >= $3 AS "enough!" FROM Dicks d
                JOIN Chats c ON d.chat_id = c.id
                WHERE (c.chat_id = $1::bigint OR c.chat_instance = $1::text)
                    AND uid = $2"#,
            chat_id.value() as String,
        ...
        .await
        .context(format!(
            "couldn't check the dick {chat_id}, {user_id} to have at least {length} cm"
        ))
    }
    ...
    pub async fn move_length(
        ...
        let mut tx = self.pool.begin().await?;
        let length_from =
            Self::move_length_for_one_user(&mut tx, internal_chat_id, from.0, -(length as i32))
                .await?;
        let length_to =
            Self::move_length_for_one_user(&mut tx, internal_chat_id, to.0, length as i32).await?;
        tx.commit().await?;

        let pos_from = self
            .get_position_in_top(internal_chat_id, from.0 as i64)
            .await?;
        let pos_to = self
            .get_position_in_top(internal_chat_id, to.0 as i64)
            .await?;
    ...
    pub async fn grow_no_attempts_check(
        ...
        let new_length =
            Self::grow_no_attempts_check_internal(&self.pool, chat_internal_id, uid, change)
                .await?
                .ok_or(anyhow!(
                    "couldn't find a dick of ({chat_id}, {uid}) for some reason"
                ))?;
    ...
    async fn insert_to_dod_table(
        ...
        .await
        .context(format!(
            "couldn't insert to DOD table for {chat_id_internal} and {user_id}"
        ))?;
        Ok(())
    }
