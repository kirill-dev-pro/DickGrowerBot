...
async fn upsert_chat() {
    ...
    sqlx::query!("DROP TRIGGER IF EXISTS trg_check_and_update_dicks_timestamp ON Dicks")
        .execute(&db)
        .await
        .expect("couldn't drop the trigger");
...
async fn clear_dicks_and_chats(db: &Pool<Postgres>) {
    sqlx::query!("DELETE FROM Dicks")
        .execute(db)
        .await
        .expect("couldn't delete dicks");
    sqlx::query!("DELETE FROM Chats")
        .execute(db)
        .await
        .expect("couldn't delete chats");
}

async fn old_chat_id_new_instance(chats: &repo::Chats, full: ChatIdFull) {
    let (id, inst) = (full.id, full.instance.clone());
    chats
        .upsert_chat(&ChatIdPartiality::Specific(id.into()))
        .await
        .expect("couldn't create a chat");

    chats
        .upsert_chat(&full.to_partiality(Default::default()))
        .await
        .expect("couldn't update the chat");
...
async fn old_instance_new_chat_id(chats: &repo::Chats, full: ChatIdFull) {
    let (id, inst) = (full.id, full.instance.clone());
    chats
        .upsert_chat(&ChatIdPartiality::Specific(inst.clone().into()))
        .await
        .expect("couldn't create a chat");
...
async fn two_separate_chats(db: &Pool<Postgres>, chats: &repo::Chats, full: ChatIdFull) {
    ...
    let ids = sqlx::query_scalar!(
        "INSERT INTO Chats (chat_id, chat_instance) VALUES ($1, NULL), (NULL, $2) RETURNING id",
        id.0,
        &inst
    )
    .fetch_all(db)
    .await
    .expect("couldn't create chats");
    assert_eq!(ids.len(), 2);
    sqlx::query!(
        "INSERT INTO Dicks (uid, chat_id, length) VALUES ($1, $2, 1), ($1, $3, 2)",
        UID,
        ids[0],
        ids[1]
    )
    .execute(db)
    .await
    .expect("couldn't create dicks");

    let chat_id = full.to_partiality(Default::default());
    dicks
        .create_or_grow(UserId(UID as u64), &chat_id, 0)
        .await
    ...
    let dick = dicks
        .get_top(&chat_id_kind, 0, 1)
        .await
        .expect("couldn't fetch the dick");
...
async fn check_chat(chats: &repo::Chats, chat_id: ChatId, inst: String) {
    let chat = chats
        ...
        .await
        .expect("couldn't fetch the chat");
    assert!(chat.is_some());
