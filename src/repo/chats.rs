use super::{ensure_only_one_row_updated, ChatIdFull, ChatIdKind, ChatIdPartiality, ChatIdSource};
...
impl TryInto<ChatIdPartiality> for Chat {
    ...
    fn try_into(self) -> Result<ChatIdPartiality, Self::Error> {
        match (self.chat_id, self.chat_instance) {
            ...
            (None, Some(instance)) => {
                Ok(ChatIdPartiality::Specific(ChatIdKind::Instance(instance)))
            }
...
repository!(
    ...
    pub async fn get_chat(&self, chat_id: ChatIdKind) -> anyhow::Result<Option<Chat>> {
        sqlx::query_as!(
            Chat,
            "SELECT id as internal_id, chat_id, chat_instance FROM Chats
        ...
        .await
        .context(format!(
            "couldn't get the information about the chat with id = {chat_id}"
        ))
    },
    pub async fn get_internal_id(
        &self,
        chat_id: &ChatIdKind,
    ) -> Result<i64, SearchError<ChatIdKind>> {
        self.get_chat(chat_id.clone())
            .await
    ...
    pub async fn upsert_chat(&self, chat_id: &ChatIdPartiality) -> anyhow::Result<i64> {
        let (id, instance) = match chat_id {
            ChatIdPartiality::Both(full, _) if self.features.chats_merging => {
                (Some(full.id.0), Some(full.instance.to_owned()))
            }
            ChatIdPartiality::Both(full, ChatIdSource::Database) => (Some(full.id.0), None),
            ChatIdPartiality::Both(full, ChatIdSource::InlineQuery) => {
                (None, Some(full.instance.clone()))
            }
            ChatIdPartiality::Specific(ChatIdKind::ID(id)) => (Some(id.0), None),
            ChatIdPartiality::Specific(ChatIdKind::Instance(instance)) => {
                (None, Some(instance.to_owned()))
            }
        };
        ...
        let internal_id = match chats.len() {
            1 if chats[0].chat_id == id && chats[0].chat_instance == instance => {
                Ok(chats[0].internal_id)
            }
    ...
    async fn create_chat(
        ...
        log::info!(
            "creating a chat with chat_id = {chat_id:?} and chat_instance = {chat_instance:?}"
        );
    ...
    async fn merge_chats(
        ...
        let deleted_dicks = sqlx::query!("DELETE FROM Dicks WHERE chat_id = $1", state.deleted.0)
            ...
            .await
            .context(format!(
                "couldn't delete dicks from the old chat with id = {}",
                state.deleted.0
            ))?
            .rows_affected();
        log::info!(
            "merging chats: {chats:?}, updated dicks: {updated_dicks}, deleted: {deleted_dicks}"
        );
        ...
        .context(format!(
            "couldn't delete the old chat with id = {} and chat_instance = {}",
            state.deleted.0, state.deleted.1
        ))?;
        sqlx::query!(
            "UPDATE Chats SET chat_instance = $3 WHERE id = $1 AND chat_id = $2",
            state.main.internal_id,
            state.main.chat_id,
            state.main.chat_instance
        )
        .execute(&mut **tx)
        .await
        .map_err(Into::into)
        .and_then(ensure_only_one_row_updated)
        .context(format!(
            "couldn't update chat_instance of the new chat with ids = {} and {} to {}",
...
#[cfg(test)]
mod tests {
    use super::{merge_chat_objects, Chat};
    ...
    fn merge_valid_chats() {
        ...
        let res = merge_chat_objects(&chats).expect("merge_chat_objects failed");
