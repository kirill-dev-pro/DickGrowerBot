...
pub static CMD_PRIVACY_COUNTER: Lazy<Counter> = Lazy::new(|| {
    Counter::new(
        "command_privacy",
        Opts::new(
            "command_privacy_usage_total",
            "count of /privacy invocations",
        ),
    )
});
pub static CMD_GROW_COUNTER: Lazy<BothModesCounters> = Lazy::new(|| {
    ...
    BothModesCounters {
        chat: Counter::new(
            "command_grow (chat)",
            opts.clone().const_label("mode", "chat"),
        ),
...
pub static CMD_LOAN_COUNTER: Lazy<BothModesComplexCommandCounters> = Lazy::new(|| {
    ...
    BothModesComplexCommandCounters {
        invoked: BothModesCounters {
            chat: Counter::new(
                "command_loan (chat)",
                invoked_opts.clone().const_label("mode", "chat"),
            ),
            inline: Counter::new(
                "command_loan (inline)",
                invoked_opts.const_label("mode", "inline"),
            ),
        },
        finished: Counter::new(
            "command_loan (finished)",
            opts.const_label("state", "finished")
...
pub static CMD_DOD_COUNTER: Lazy<BothModesCounters> = Lazy::new(|| {
    let opts = Opts::new(
        "command_dick_of_day_usage_total",
        "count of /dick_of_day invocations",
    );
    BothModesCounters {
        chat: Counter::new(
            "command_dick_of_day (chat)",
            opts.clone().const_label("mode", "chat"),
        ),
        inline: Counter::new(
            "command_dick_of_day (inline)",
            opts.const_label("mode", "inline"),
        ),
    }
});
...
pub static CMD_IMPORT: Lazy<ComplexCommandCounters> = Lazy::new(|| {
    let opts = Opts::new(
        "command_import_usage_total",
        "count of /import invocations and successes",
    );
    ComplexCommandCounters {
        invoked: Counter::new(
            "command_import (invoked)",
            opts.clone().const_label("state", "invoked"),
        ),
        finished: Counter::new(
            "command_import (finished)",
            opts.const_label("state", "finished"),
        ),
    }
});
pub static CMD_PROMO: Lazy<DeepLinkedCommandsCounters> = Lazy::new(|| {
    let opts = Opts::new(
        "command_promo_usage_total",
        "count of /promo invocations and successes",
    );
    DeepLinkedCommandsCounters {
        invoked_by_command: Counter::new(
            "command_promo (invoked)",
            opts.clone().const_label("state", "invoked_by_command"),
        ),
        invoked_by_deeplink: Counter::new(
            "deeplink_promo (invoked)",
            opts.clone().const_label("state", "invoked_by_deeplink"),
        ),
        finished: Counter::new(
            "command_promo (finished)",
            opts.const_label("state", "finished"),
        ),
    }
});
...
impl Registry {
    fn register(&self, counter: &Counter) -> &Self {
        self.0
            .register(Box::new(counter.inner.clone()))
